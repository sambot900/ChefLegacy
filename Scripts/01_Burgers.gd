...
func perform_interaction(cname):
	...
	if p_state[0] == 1: # player is enabled
		if i_state[0] == 1: # interactable is enabled
			if i_state[2] == 1: # interactable is laden
				state_change(cname, [1,0,1])
				if (p_state == [1,0,0]) or (p_state == [1,1,0]) or (p_state == [1,0,1]): # player has one empty hand
					print("pick up?")
					if p_state == [1,0,0]:
						state_change(cname, [1,1,0])
					elif p_state == [1,1,0]:
					pass # pick up?
